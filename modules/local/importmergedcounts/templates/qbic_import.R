...
# Rewrite input tables to a standard differential abundance input; columns in sample sheet need to be renamed from file names to QBiC Codes
col_names <- list(colnames(count.table))[[1]]
remaining_names <- col_names
qbic_codes <- sample.sheet["QBiC.Code"][[1]]
for (c in c(1:length(col_names))) {
    for (q in c(1:length(qbic_codes))) {
        if (grepl(toString(qbic_codes[q]), col_names[[c]], fixed = TRUE)) {
            old_name <- col_names[[c]]
            new_name <- toString(qbic_codes[q])
