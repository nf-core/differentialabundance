...
# check parameters
if (!opt\$metric %in% c('rho', 'phi', 'phs', 'pcor.bshrink', 'cor', 'vlr', 'pcor', 'pcor.shrink')) {
    stop('Only vlr, rho, phi, phs, pcor.bshrink are allowed for logratio analysis. And only cor, pcor, pcor.bshrink are allowed for rest of anaysis. Please make sure you provided the correct metric')
}
if (opt\$metric == 'pcor.bshrink'){
    ...
    if (!opt\$transformation %in% c('clr', 'alr', NA)) stop('Please make sure you provided the correct transformation: clr or alr.')
    if (is.na(opt\$transformation)) print('No transformation is required by user. We assume the input count data was already properly transformed.')
}
