...
- [[#710](https://github.com/nf-core/differentialabundance/issues/710)] - Regenerate pipeline metro map against current `dev` using [nf-metro](https://github.com/pinin4fjords/nf-metro). Replaces `docs/images/workflow.{png,svg}` with `docs/images/nf-core-differentialabundance_metro_map.{png,svg}` (plus an animated SVG variant), and adds `assets/metro_map.mmd` as the source and `docs/dev/metro_map.md` with regeneration instructions. README embeds the animated SVG with a static PNG fallback, matching nf-core/rnaseq ([@pinin4fjords](https://github.com/pinin4fjords)).
