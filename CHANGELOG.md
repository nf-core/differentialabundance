...
- [[#719](https://github.com/nf-core/differentialabundance/issues/719)] - Fix double-underscore and separator output filename inconsistency. ([@tracelail](https://github.com/tracelail)).
...
- [[#717](https://github.com/nf-core/differentialabundance/issues/717)] - Fix `-stub-run` aborting in `splitCsv` by bumping `shinyngs/validatefomcomponents` to a version that emits header-bearing stub TSVs ([nf-core/modules#11628](https://github.com/nf-core/modules/pull/11628)) ([@pinin4fjords](https://github.com/pinin4fjords), review by [@atrigila](https://github.com/atrigila)).
- [[#713](https://github.com/nf-core/differentialabundance/issues/713)] - Workflow robustness: rewrite matrix-copy ternary as an explicit conditional and add a fallback `other` branch to `ch_multi_validated_assays` ([@pinin4fjords](https://github.com/pinin4fjords), review by [@atrigila](https://github.com/atrigila)).
...
- [[#710](https://github.com/nf-core/differentialabundance/issues/710)] - Regenerate pipeline metro map against current `dev` using [nf-metro](https://github.com/pinin4fjords/nf-metro). Replaces `docs/images/workflow.{png,svg}` with `docs/images/nf-core-differentialabundance_metro_map.{png,svg}` (plus an animated SVG variant), and adds `assets/metro_map.mmd` as the source and `docs/dev/metro_map.md` with regeneration instructions ([@pinin4fjords](https://github.com/pinin4fjords), review by [@apeltzer](https://github.com/apeltzer)).
- [[#729](https://github.com/nf-core/differentialabundance/pull/729)] - Consolidate the default Quarto report (`assets/differentialabundance_report.qmd`) into a single helpers chunk plus a thin orchestration body; modernise to tidyverse idioms (native `|>`, `purrr::map_*`/`walk`, `\(x)`); fix a metadata-column NA-coercion bug, render filtering-summary text as prose rather than a code block, and correct the `assocations` typo. Also adds a "Quick start with the test profile" subsection to `docs/usage.md` ([@pinin4fjords](https://github.com/pinin4fjords), review by [@atrigila](https://github.com/atrigila)).
...
- [[#638](https://github.com/nf-core/differentialabundance/pull/638)] - Remove `--report_scree` parameter and replace with `--disable_report_modules` to make report more customizable ([@delfiterradas](https://github.com/delfiterradas), review by [@grst](https://github.com/grst) and [@pinin4fjords](https://github.com/pinin4fjords)).
