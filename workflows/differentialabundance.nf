...
if (params.report_file) {report_file = file(params.report_file, checkIfExists: true) }
...
workflow DIFFERENTIALABUNDANCE {
    ...
    ch_fom = Channel.of(exp_meta).combine(ch_input).combine(GTF_TO_TABLE.out.feature_annotation.map{it[1]}).combine(ch_counts)
    ...
    if (params.vs_method.split(',').contains('vst')) {
        ch_processed_matrices = ch_processed_matrices.join(DESEQ2_DIFFERENTIAL.out.vst_counts)
    }
    if (params.vs_method.split(',').contains('rlog')) {
        ch_processed_matrices = ch_processed_matrices.join(DESEQ2_DIFFERENTIAL.out.rlog_counts)
    }
    ...
    // Gather software versions for the modules that require a GTF
