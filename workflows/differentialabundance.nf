...
if (run_gene_set_analysis) {
    if (params.gene_sets_files) {
        ...
        if (!params.gene_sets_files && !params.gprofiler2_organism) {
            error("To run gprofiler2, please provide a run token, GMT file or organism!")
        } else if (params.gprofiler2_mode == "token" && !params.gprofiler2_token) {
            error("--gprofiler2_mode set to 'token' but no --gprofiler2_token provided!")
        } else if (params.gprofiler2_mode == "organism" && !params.gprofiler2_organism) {
...
workflow DIFFERENTIALABUNDANCE {
    ...
    if (params.gprofiler2_run) {
        ...
        ch_gprofiler_gene_sets = ch_gene_sets
        if (params.gprofiler2_mode == "token" || params.gprofiler2_mode == "organism") {
