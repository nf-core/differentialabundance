...
dg <- read_metadata(file.path(params$input_dir, params$detected_genes))
multiqc <- TidyMultiqc::load_multiqc(file.path(params$input_dir, params$multiqc_json))
```
...
p_value_types <- list(Adjusted = params$differential_qval_column)
p_value_thresholds <- list(Adjusted = params$differential_max_qval)
      ...
      function(x){
        ...
        list(
          up = differential_results[[x]][which(
            differential_results[[x]][,params$differential_fc_column ] > round(log2(params$differential_min_fold_change),2) &
            signif
          ),],
          down = differential_results[[x]][which(
            differential_results[[x]][,params$differential_fc_column ] < round(log2(1/params$differential_min_fold_change),2) &
            signif 
          ),]
        ) 
      }
...
multiqc <-
  multiqc |> 
  rename("SampleName" = metadata.sample_id) |> 
  mutate(SampleName = str_remove(SampleName, "_S\\d+_\\w+"))

observations_to_print <- observations[,unique(display_columns)] |>
                            left_join(dg) |>
                            left_join(multiqc, by = "SampleName") |>
                            dplyr::select(SampleName,
                                            Original_Sample_Name, 
                                            Condition,
                                            general.total_sequences, 
                                            general.uniquely_mapped, 
                                            general.multimapped, 
                                            Detected_Genes) |>
                            rename("NEGEDIA_Barcode" = SampleName,
                                    "Sample_Name" = Original_Sample_Name,
                                    "Total_Sequences" = general.total_sequences,
                                    "Uniquely_Mapped" = general.uniquely_mapped,
                                    "Multimapped" = general.multimapped)
...
Comparisons were made between `r params$observations_type` groups defined using `r params$observations_type` metadata columns, as described in the following table of contrasts:
...
Input was a matrix of `r nrow(assay_data$raw)` `r params$features_type`s for `r ncol(assay_data$raw)` Samples`r ifelse(nrow(assay_data$normalised) < nrow(assay_data$raw), paste0(', reduced to ', nrow(assay_data$normalised), ' ', params$features_type, 's after filtering for low abundance'), '')`.
...
### Differential `r params$features_type` `r params$study_abundance_type`
...
for (i in 1:nrow(contrasts)){
  ...
  for (pvt in names(p_value_types)){
    ...
    vline_thresholds[[paste(params$differential_fc_column, '<', round(-log2(params$differential_min_fold_change),2))]] = round(-log2(params$differential_min_fold_change),2)
    vline_thresholds[[paste(params$differential_fc_column, '>', round(log2(params$differential_min_fold_change),2))]] = round(log2(params$differential_min_fold_change),2)
