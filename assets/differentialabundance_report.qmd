...
for (i in 1:nrow(contrasts)) {
  ...
  for (pvt in names(p_value_types)) {
    ...
    vline_thresholds <- list(
      setNames(-log2(params$meta$params$differential_min_fold_change),
               paste(params$meta$params$differential_fc_column, "<=", log2(params$meta$params$differential_min_fold_change))),
      setNames( log2(params$meta$params$differential_min_fold_change),
               paste(params$meta$params$differential_fc_column, ">=", log2(params$meta$params$differential_min_fold_change)))
    )

    palette_volcano <- append("#999999", makeColorScale(3, params$meta$params$differential_palette_name))
    ...
    # Put Direction first and prettify
    ...
    combined <- round_dataframe_columns(combined, digits = params$meta$params$round_digits)
    ...
    for (assay_type in rev(names(assay_data))) {
      ...
      # subset observations to samples in expr_mat (columns are already renamed to observation names)
