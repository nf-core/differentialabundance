...
render_pca_meta_associations <- function() {
    ...
    # Wrap in a tagList so `print` goes through the shiny.tag.list method
    # (which writes the widget HTML via `cat(format(...))`) instead of
    # `print.htmlwidget`, which would only open an interactive viewer.
    # The widget's JS dependencies are pre-registered via touch-js-libs.
    print(htmltools::tagList(d3heatmap::d3heatmap(
        -log10(plot_pca_meta),
        Rowv = FALSE,
        dendrogram = "none",
...
d3heatmap::d3heatmap(matrix(0, 2, 2))
```
