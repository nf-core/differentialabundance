...
for (assay_type in rev(names(assay_data))){
    ...
    for (iv in informative_variables) {
        ...
        dendroColorScale <- rep(dendroColorScale, length.out = length(unique(observations[[iv]])))

        # assign names matching the category labels
        names(dendroColorScale) <- sort(unique(na.omit(as.character(observations[[iv]]))))
    ...
    heat_mat <- 2^assay_data[[assay_type]][variable_genes, ]
    p <- Heatmap(
        heat_mat,
        name = "Expression",
        top_annotation = ha,
        ...
        clustering_method_rows = params$meta$params$exploratory_clustering_method,
        clustering_distance_columns = params$meta$params$exploratory_cor_method,
        clustering_distance_rows = params$meta$params$exploratory_cor_method,
        column_title = paste0(
