...
for (i in 1:nrow(contrasts)) {
    ...
    for (pvt in names(p_value_types)) {
        ...
        vline_thresholds <- list(
            setNames(-log2(params$meta$params$differential_min_fold_change), paste(params$meta$params$differential_fc_column, "<=", log2(params$meta$params$differential_min_fold_change))),
            setNames( log2(params$meta$params$differential_min_fold_change), paste(params$meta$params$differential_fc_column, ">=", log2(params$meta$params$differential_min_fold_change)))
        )
        ...
        if (sum(de_fc_sig) > 0) {
            combined <- full_de[de_fc_sig, ]  # Filter to only significant genes
            # Add Direction column based on fold-change
            combined$Direction <- ifelse(
                combined[[params$meta$params$differential_fc_column]] > 0,
