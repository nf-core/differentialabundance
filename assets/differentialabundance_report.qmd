...
for (i in 1:nrow(contrasts)) {
    ...
    p_value_types <- list(
        Adjusted   = params$meta$params$differential_qval_column,
        Unadjusted = params$meta$params$differential_pval_column
  )
  p_value_thresholds <- list(
    Adjusted   = params$meta$params$differential_max_qval,
    Unadjusted = params$meta$params$differential_max_pval
  )
  ...
  for (pvt in names(p_value_types)) {
    ...
    de_fc <- abs(full_de[[params$meta$params$differential_fc_column]]) >=
      abs(log2(params$meta$params$differential_min_fold_change))
    ...
    full_de$differential_status <- factor(
      ...
      ordered = TRUE
        ...
        vline_thresholds[[paste(params$meta$params$differential_fc_column, ">=", log2(params$meta$params$differential_min_fold_change))]] <- log2(params$meta$params$differential_min_fold_change)
        palette_volcano <- append(c("#999999"), makeColorScale(3, params$meta$params$differential_palette_name))
    ...
    plot_args <- list(
      ...
      xlab = paste("higher in", contrasts$reference[i], "<<", params$meta$params$differential_fc_column, ">>", "higher in", contrasts$target[i]),
    ...
    if (zero_p) {
      cat(sprintf("<i>%d feature%s not shown because of p value = 0; please refer to the results tables.</i><br><br>",
        ...
        if (sum(de_fc_sig) > 0) {
            ...
            # Now format the numeric columns for display
            ...
            if (length(cols_present_pretty) > 0) {
                dt <- formatRound(dt, cols_present_pretty, digits = 5)
            }
