...
if (! is.null(params$features)){
    ...
    if (! is.null(params$meta$params$features_metadata_cols)){
        features <- features[,colnames(features) %in% simpleSplit(params$meta$params$features_metadata_cols), drop = FALSE]
    }
}
...
```{r, echo=FALSE, results='asis'}
display_columns <- union(c(params$meta$params$observations_id_col, unique(contrasts$variable)), informative_variables)
...
for (i in 1:nrow(contrasts)){
    ...
    if (! is.null(params$features) && (! is.null(params$meta$params$differential_feature_name_column)) ){
