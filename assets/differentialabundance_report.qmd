...
if (!is.null(params$filtering_tests)) {
    filtering_tests_path <- file.path(params$input_dir, params$filtering_tests)

    if (file.exists(filtering_tests_path) && file.size(filtering_tests_path) > 0) {
        filtering_tests <- read_metadata(filtering_tests_path)

        if (nrow(filtering_tests) > 0 && ncol(filtering_tests) > 1) {
           test_cols <- colnames(filtering_tests)[-1]

            test_values <- filtering_tests[, test_cols, drop = FALSE]
            test_values <- data.frame(lapply(test_values, function(x) {
            ...
            total_features <- nrow(test_values)

            pass_all <- apply(test_values, 1, function(r) {
                r[is.na(r)] <- FALSE
                all(r)
            })

            retained_n <- sum(pass_all)
            removed_n  <- total_features - retained_n

            failure_matrix <- data.frame(!test_values, check.names = FALSE)
            failure_matrix[is.na(failure_matrix)] <- FALSE

            failure_any <- colSums(failure_matrix)
            failure_only <- sapply(test_cols, function(col) {
                sum(failure_matrix[, col] & rowSums(failure_matrix) == 1)
            })
            ...
            rule_display <- ifelse(test_cols %in% names(rule_labels), rule_labels[test_cols], test_cols)

            n_samples <- nrow(observations)

            # minimum_samples (can be overridden by grouping_var or proportion, like the script)
            min_samples_abundance <- params$meta$params$filtering_min_samples
            if (is.null(min_samples_abundance)) min_samples_abundance <- 1

            if (!is.null(params$meta$params$filtering_grouping_var) &&
                params$meta$params$filtering_grouping_var %in% colnames(observations)) {

                gv <- params$meta$params$filtering_grouping_var
                min_samples_abundance <- min(table(observations[[gv]]))

                if (!is.null(params$meta$params$filtering_min_proportion) &&
                    params$meta$params$filtering_min_proportion > 0) {
                    min_samples_abundance <- min_samples_abundance * params$meta$params$filtering_min_proportion
                }

            } else if (!is.null(params$meta$params$filtering_min_proportion) &&
                       params$meta$params$filtering_min_proportion > 0) {

                min_samples_abundance <- n_samples * params$meta$params$filtering_min_proportion
            }

            min_samples_abundance <- ceiling(min_samples_abundance)

            # minimum_samples_not_na (uses min_samples_not_na if given, else proportion_not_na)
            min_samples_not_na <- params$meta$params$filtering_min_samples_not_na
            if (is.null(min_samples_not_na)) {
                prop_not_na <- params$meta$params$filtering_min_proportion_not_na
                if (is.null(prop_not_na)) prop_not_na <- 0.5
                min_samples_not_na <- n_samples * prop_not_na
            }
            min_samples_not_na <- ceiling(min_samples_not_na)

            threshold_used <- sapply(test_cols, function(col) {
                if (col == "abundance") {
                    paste0(
                        "abundance ≥ ", params$meta$params$filtering_min_abundance,
                        " in ≥ ", min_samples_abundance, " samples"
                    )
                } else if (col == "na") {
                    paste0("non-NA ≥ ", min_samples_not_na, " samples")
                } else if (col == "most_variant_vectors") {
                    paste0("top ", params$meta$params$filtering_most_variant_features, " most-variant")
                } else {
                    NA
                }
            })
            ...
            summary_table <- data.frame(
                ...
                Count   = c(total_features, retained_n, removed_n),
                Percent = c(100, retained_n / total_features * 100, removed_n / total_features * 100)
            )

            summary_table <- round_dataframe_columns(summary_table, digits = params$meta$params$round_digits)

            rule_table <- data.frame(
                Rule = rule_display,
                Threshold = threshold_used,
                `Failed (any)` = failure_any,
                `Failed (only this rule)` = failure_only,
                check.names = FALSE
            )

            cat("The following tables summarize how many features were retained or removed and which filtering rules caused failures.\n\n")
            ...
            cat("Filtering statistics were requested, but no test columns were found in the filtering tests file.\n")
        }
    } else {
        cat("Filtering statistics were requested, but the filtering tests file was missing or empty.\n")
    }
}
