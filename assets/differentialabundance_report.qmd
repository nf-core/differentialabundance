...
module_disabled <- function(x) {
  mods <- params$meta$params$disable_report_modules
  if (is.null(mods) || mods == "") {
    return(FALSE)
  } else {
    x %in% simpleSplit(mods)
  }
}

generate_boxplots <- function(combined, assay_data, observations, params,
                               contrast_variables, main_grouping_variable,
                               groupColorScale, label_col) {
    qval_pretty <- prettifyVariablename(params$meta$params$differential_qval_column)

    # choose ID + display label columns in 'combined' after prettify
    features_id_pretty <- prettifyVariablename(params$meta$params$features_id_col)

    if (!is.null(params$meta$params$features_name_col) &&
        prettifyVariablename(params$meta$params$features_name_col) %in% colnames(combined)) {
        feature_label_pretty <- prettifyVariablename(params$meta$params$features_name_col)
    } else {
    ...
    if (qval_pretty %in% colnames(combined)) {
        ...
        top_ids <- head(combined[[features_id_pretty]][ord], 10)
        top_ids <- as.character(top_ids)
    } else {
        # fallback: take first 10
        top_ids <- head(as.character(combined[[features_id_pretty]]), 10)
    }
    ...
    for (assay_type in rev(names(assay_data))) {
        ...
        # subset observations to samples in expr_mat
        obs_sub <- observations[colnames(expr_mat), , drop = FALSE]

        plot_df <- as.data.frame(expr_mat[top_present, , drop = FALSE])
        plot_df[[prettifyVariablename(params$meta$params$features_type)]] <- rownames(plot_df)

        features_pretty <- prettifyVariablename(params$meta$params$features_type)

        plot_df <- tidyr::pivot_longer(
        plot_df,
        cols = -all_of(features_pretty),
        names_to = "Sample",
        values_to = "Expression"
        )
        plot_df <- dplyr::arrange(plot_df, .data[[features_pretty]])

        # Condition: use first contrast variable if present; else use main grouping var
        ...
        # Map feature IDs -> display names (symbol if available)
        ...
        # Map feature IDs -> qvals for label
        if (qval_pretty %in% colnames(combined)) {
        ...
        # Panel-specific y positions (avoid global max overlap)
        y_pos <- plot_df %>%
        dplyr::group_by(FeatureName) %>%
        ...
        p_box <- ggplotly(p_box, tooltip = c("x", "y", "color", "text")) %>%
        layout(height = 600)
...
make_params_table <- function(name, pattern = NULL, remove_pattern = FALSE) {
  ...
  caption_text <- paste("Parameters used for", name)
...
caption_text <- paste(
  ucfirst(params$meta$params$observations_type),
  "metadata"
)
...
caption_text <- "Table of contrasts"
...
rt <- reactable(
  ...
  columns = list(
    Id = reactable::colDef(
      minWidth = 350,
      style = list(wordBreak = "break-word")
    )
  )
)
...
summary_tbl <- tibble(
  ...
  Percent = c(
    100,
    100 * retained / total_features,
    100 * removed / total_features
  )
)
...
caption_text <- "Filtering summary"
...
caption_text <- "Filtering rule breakdown (counts may overlap)"
...
```{r, echo=FALSE, fig.height=8, eval=!module_disabled("boxplot")}
#| echo: false
...
```{r, echo=FALSE, fig.height=8, eval=!module_disabled("density")}
#| echo: false
...
```{r, echo=FALSE, results='asis', eval=!module_disabled("pca")}
#| echo: false
...
```{r, echo=FALSE, results='asis', eval=!module_disabled("scree")}
cat(paste0("\n#### Scree plot\n"))
cat("::: {.panel-tabset}")
...
for (assay_type in names(percentVar_list)) {
    ...
    cat(paste0("\n##### ", prettifyVariablename(assay_type), "\n"))
    print(
        htmltools::tagList(plot_screeplot(percentVarData))
    )
    cat("\n")
}
cat(":::")
...
```{r, eval=!module_disabled("heatmap")}
#| echo: false
#| results: asis
#| warning: false
#| message: false
...
```{r, echo=FALSE, results='asis', warning=FALSE, eval=!module_disabled("outlier")}
...
#| echo: false
#| results: asis
# Add method-specific differential analysis details to the report
method_description <- switch(
    params$meta$params$differential_method,
    "deseq2" = paste0(
        "The `DESeq2 R` package was used for differential analysis. p-values were adjusted with the ", params$meta$params$deseq2_p_adjust_method,
        " method to reduce the number of false positives. ", ucfirst(params$meta$params$features_type),
        "s were considered differential if, for the respective contrast, the adjusted p-value was equal to or lower than ", params$meta$params$deseq2_alpha,
        " and the absolute log2 fold change was equal to or higher than ", params$meta$params$deseq2_lfc_threshold, "."
    ),
    "limma" = paste0(
        "The `limma R` package", ifelse(isTRUE(params$meta$params$limma_use_voom), " with `voom` transformation", ""),
        " was used for differential analysis. p-values were adjusted with the ", params$meta$params$limma_adjust_method,
        " method to reduce the number of false positives. ", ucfirst(params$meta$params$features_type),
        "s were considered differential if, for the respective contrast, the adjusted p-value was equal to or lower than ", params$meta$params$limma_p_value,
        " and the absolute log2 fold change was equal to or higher than ", params$meta$params$limma_lfc, "."
    ),
    "dream" = paste0(
        "The `variancePartition` `dream` workflow", ifelse(isTRUE(params$meta$params$dream_apply_voom), " with `voom` transformation", ""),
        " was used for differential analysis. p-values were adjusted with the ", params$meta$params$dream_adjust_method,
        " method to reduce the number of false positives. ", ucfirst(params$meta$params$features_type),
        "s were considered differential if, for the respective contrast, the adjusted p-value was equal to or lower than ", params$meta$params$dream_p_value,
        " and the absolute log2 fold change was equal to or higher than ", params$meta$params$dream_lfc, "."
    ),
    paste0(
        "The `", params$meta$params$differential_method, "` method was used for differential analysis."
    )
)
...
foo <- lapply(names(p_value_types), function(pvt) {
    ...
    params$meta$params$abundance_type,
...
```{r, echo=FALSE, results='asis', warning=FALSE, eval=!module_disabled("upset_plot")}
#| echo: false
#| results: asis
#| output: asis
#| warning: false
...
for (i in 1:nrow(contrasts)) {
    ...
    needed  <- c(params$meta$params$differential_fc_column,
                params$meta$params$differential_qval_column,
                params$meta$params$differential_pval_column)
    ...
    for (pvt in names(p_value_types)) {
        ...
        if (!module_disabled("volcano")) {
            p <- do.call(plotly_scatterplot, plot_args) %>%
                layout(xaxis = list(range = list(-max_fc, max_fc))) %>%
                add_segments(
                    x = -max_fc,
                    xend = max_fc,
                    y = -log10(p_value_thresholds[[pvt]]),
                    yend = -log10(p_value_thresholds[[pvt]]),
                    line = list(color = "black", width = 2, dash = "dot"),
                    showlegend = TRUE,
                    inherit = FALSE,
                    name = paste(pval_column, "=", p_value_thresholds[[pvt]])
                )
        ...
        # Filter full_de to get only significant features and add Direction column
        de_fc_sig <- de_fc & de_pval  # Genes that meet BOTH criteria

        if (sum(de_fc_sig) > 0) {
            combined <- full_de %>%
            filter(de_fc_sig) %>%
            select(-differential_status) %>%
            mutate(
                Direction = if_else(
                    .data[[params$meta$params$differential_fc_column]] > 0,
                    "Up",    # Positive fold-change = Up-regulated
            ...
            # Identify numeric columns that need formatting
            cols_to_format_pretty <- prettifyVariablename(c(
                params$meta$params$differential_fc_column,
                params$meta$params$differential_pval_column,
                params$meta$params$differential_qval_column
            ))
            ...
            for (col_name in colnames(combined)) {
                if (col_name %in% cols_present_pretty) {
                    # Format numeric columns with 5 digits
                    column_defs[[col_name]] <- colDef(
                        format = colFormat(digits = 5),
                        filterable = !(col_name %in% no_filter_cols),
                        align = "right"
                    )
                } else if (col_name == "Direction") {
                    # Special formatting for Direction column
                    column_defs[[col_name]] <- colDef(
                        filterable = TRUE,
                        align = "center"
                    )
            ...
            if (!module_disabled("de_table")) {
                ...
                # Create the reactable with caption
                rt <- reactable(
                    combined,
                    columns = column_defs,
                    filterable = TRUE,
                    ...
                    defaultSorted = setNames(
                        list("asc"),
                        prettifyVariablename(params$meta$params$differential_qval_column)
                    )
                )

                # Add caption as HTML wrapper
            ...
            generate_boxplots(combined, assay_data, observations, params,
                  contrast_variables, main_grouping_variable,
                  groupColorScale, label_col)

            # Optional biotype plot, now grouped by Direction
            if ("Gene biotype" %in% colnames(combined) && !module_disabled("biotype_plot")) {
...
```{r, warning=FALSE, results='asis', eval=!module_disabled("gsea")}
#| echo: false
#| results: asis
#| warning: false
...
if (!is.null(params$meta$params$functional_method)) {
  ...
  # Helper to render reactable inside tabsets without breaking layout
  render_reactable <- function(df, caption_text, page_size = 10) {
    ...
    htmltools::tagList(
      # table container: prevents panel/tabset overflow issues
      htmltools::tags$div(
        style = "overflow-x: auto; max-width: 100%;",
        rt
      ),
      # caption below, centered (DT-like)
      htmltools::tags$div(
        caption_text,
        style = "text-align: center; margin-top: 10px; color: #5a677a;"
      )
  ...
  if (params$meta$params$functional_method == "gsea") {
    ...
    if (nrow(gsea_contrasts) == 0) {
      ...
      for (gmt_file in simpleSplit(params$meta$params$gene_sets_files)) {
        ...
        for (i in seq_len(nrow(gsea_contrasts))) {
          ...
          if (file.exists(target_gsea_tables[i])) {
            ...
            print(
              render_reactable(
                target_gsea_results,
                paste0("Target (", gsea_contrasts$target[i], ")")
              )
            )
          } else {
          ...
          if (file.exists(reference_gsea_tables[i])) {
            ...
            print(
              render_reactable(
                ref_gsea_results,
                paste0("Reference (", gsea_contrasts$reference[i], ")")
              )
            )
          } else {
        ...
        cat(":::\n") # close inner panel-tabset (contrasts)
      }
      cat(":::\n") # close outer panel-tabset (gmt files)
    }
    ...
    cat(paste0(
      ...
      ifelse(
        params$meta$params$gprofiler2_significant,
        paste0(" Enrichment was only considered if significant, i.e. adjusted p-value <= ", params$meta$params$gprofiler2_max_qval, "."),
    ...
    for (i in seq_along(differential_names)) {
      ...
      if (!file.exists(table_path) || file.size(table_path) == 0) {
        cat(paste0(
          "No ",
          ifelse(params$meta$params$gprofiler2_significant, "significantly", ""),
        ...
        caption_text <- paste(
          "Enriched pathways in", name,
          "(check", table, "for more detail)"
        )
...
caption_text <- "Software versions"
