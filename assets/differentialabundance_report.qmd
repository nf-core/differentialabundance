...
round_dataframe_columns <- function(df, columns = NULL, digits = -1) {
    ...
    if (is.null(columns)) {
        columns <- colnames(df)[purrr::map_lgl(df, is.numeric)]
    }
    df[, columns] <- format(
...
generate_boxplots <- function(combined, label_col) {
    ...
    for (assay_type in rev(names(assay_data))) {
        ...
        plot_df <- dplyr::arrange(plot_df, .data[[features_pretty]])
        ...
        pvals_grouped <- plot_df |>
            ...
            dplyr::summarise(pval = dplyr::first(pval), .groups = "drop")
        y_pos <- plot_df |>
            dplyr::group_by(FeatureName) |>
            dplyr::summarise(y = max(Expression, na.rm = TRUE) * 1.03, .groups = "drop")
        pvals_grouped <- dplyr::left_join(pvals_grouped, y_pos, by = "FeatureName")

        p_box <- ggplot2::ggplot(plot_df, ggplot2::aes(x = Condition, y = Expression)) +
            ggplot2::geom_boxplot(outlier.shape = NA) +
            ggbeeswarm::geom_quasirandom(
                width = 0.2,
                ggplot2::aes(color = Condition, text = paste0("Sample: ", Sample, "<br>"))
            ) +
            ggplot2::facet_wrap(~ FeatureName, scales = "free_y") +
            ggplot2::scale_color_manual(values = groupColorScale) +
            ggplot2::theme_light() +
            ggplot2::theme(
                axis.text.x = ggplot2::element_text(angle = 45, hjust = 1),
                axis.title.x = ggplot2::element_text(margin = ggplot2::margin(t = 15)),
                legend.position = "none",
                strip.text = ggplot2::element_text(color = "black", size = 8),
                ...
                plot.margin = ggplot2::margin(t = 15, b = 20)
            ) +
            ggplot2::geom_text(
                data = pvals_grouped,
                ggplot2::aes(x = 1.6, y = y, label = paste0("Padj = ", signif(pval, 3))),
                inherit.aes = FALSE,
...
render_filtering_summary <- function() {
    ...
    test_results <- tests |>
        ...
        dplyr::mutate(dplyr::across(dplyr::everything(),
                                    ~ as.logical(dplyr::na_if(trimws(as.character(.x)), ""))))
    ...
    passed_all <- test_results |>
        ...
        dplyr::mutate(pass = rowSums(dplyr::across(dplyr::everything(), as.integer)) == ncol(test_results)) |>
        dplyr::pull(pass)
    retained <- sum(passed_all)
    removed  <- total_features - retained

    failures <- test_results |>
        dplyr::mutate(dplyr::across(dplyr::everything(), ~ .x %in% FALSE))
    failure_count <- failures |>
        dplyr::mutate(n = rowSums(dplyr::across(dplyr::everything(), as.integer))) |>
        dplyr::pull(n)

    thresholds <- if (file.exists(thresholds_path) && file.size(thresholds_path) > 0) {
        readr::read_tsv(thresholds_path, show_col_types = FALSE)
    } else {
        tibble::tibble(rule = character(), threshold = numeric())
    }

    rule_map <- tibble::tibble(
    ...
    tbl <- tibble::tibble(
        ...
        dplyr::left_join(rule_map, by = "column") |>
        dplyr::left_join(thresholds, by = "rule") |>
        dplyr::mutate(
            Rule = dplyr::recode(
                column,
        ...
        dplyr::select(Rule, Threshold, `Failed (any)`, `Failed (only this rule)`)
    ...
    for (assay_type in rev(names(assay_data))) {
        ...
        for (d in names(plot_types)) {
            ...
            updatemenus <- list(list(
                ...
                x = 1, xanchor = "right",
                y = 1.1, yanchor = "top",
                buttons = purrr::map(informative_variables, \(iv) {
    ...
    for (variable in rownames(pca_vs_meta)) {
        ...
        if (any(sig_comps)) {
            ...
            cat(paste0("The variable '", variable, "' shows an association with ",
                       colnames(pca_vs_meta)[min_sig_comp], " (p = ", min_sig_comp_p, "). "))
        }
    }
}
...
render_clustering_heatmaps <- function() {
    ...
    for (assay_type in rev(names(assay_data))) {
        ...
        ann_colors <- list()
        for (iv in informative_variables) {
            if (length(unique(observations[[iv]])) > 10) next
            if (is.numeric(observations[[iv]])) {
                ...
                names(cscale) <- sort(vals)
                ann_colors[[iv]] <- cscale
            }
        }
...
render_outlier_detection <- function() {
    iv_min_group_sizes <- purrr::map_int(
    ...
    purrr::walk(informative_variables[iv_min_group_sizes > 2], \(iv) {
        cat(paste("\n####", iv, "\n"))
        plotdata <- madScore(
...
render_de_count_tables <- function() {
    cat("::: {.panel-tabset}")
    purrr::walk(names(p_value_types), \(pvt) {
        cat("\n#### ", pvt, "\n")
...
render_upset_plots <- function() {
    ...
    purrr::walk(names(p_value_types), \(pvt) {
        ...
        de_genes <- purrr::map(differential_results, \(de) {
        ...
        # Number of contrasts that have at least 2 differential features (UpSet needs >= 2 sets with content)
        de_con <- sum(!is.na(purrr::map_chr(de_genes, \(x) as.character(x[2]))))
        if (de_con > 1) {
...
build_de_combined <- function(full_de, mask) {
    full_de |>
        dplyr::filter(mask) |>
        dplyr::select(-tidyselect::any_of("differential_status")) |>
        dplyr::mutate(
            Direction = dplyr::if_else(.data[[p$differential_fc_column]] > 0, "Up", "Down")
        ) |>
        dplyr::arrange(
            .data[[p$differential_qval_column]],
            .data[[p$differential_pval_column]]
        ) |>
        dplyr::relocate(Direction) |>
        dplyr::rename_with(prettifyVariablename)
}
...
render_biotype_plot <- function(combined) {
    ...
    biotype_plot <- ggplot2::ggplot(
            bt,
            ggplot2::aes(x = stats::reorder(`Gene biotype`, -count), y = count, fill = Direction)
        ) +
...
render_de_details <- function(i) {
    ...
    for (pvt in names(p_value_types)) {
        ...
        if (sum(de_fc_sig) > 0) {
            ...
            if (!module_disabled("biotype_plot")) {
                render_biotype_plot(combined)
            }
        } else {
...
render_gsea_section <- function() {
    ...
    cat("::: {.panel-tabset}\n")
    for (gmt_file in simpleSplit(p$gene_sets_files)) {
        gmt_name <- basename(tools::file_path_sans_ext(gmt_file))
        cat("\n##### ", gmt_name, "\n")
        cat("::: {.panel-tabset}\n")

        ref_tables <- paste0(gsea_contrasts$id, ".", gmt_name, ".gsea_report_for_", gsea_contrasts$reference, ".tsv")
        tgt_tables <- paste0(gsea_contrasts$id, ".", gmt_name, ".gsea_report_for_", gsea_contrasts$target,    ".tsv")

        for (i in seq_len(nrow(gsea_contrasts))) {
            cat("\n###### ", contrast_descriptions[i], "\n")

            if (file.exists(tgt_tables[i])) {
                tgt <- read_metadata(tgt_tables[i])[, c(-2, -3)]
                tgt <- round_dataframe_columns(tgt, digits = p$round_digits)
                print(captioned_table(tgt, caption = paste0("Target (", gsea_contrasts$target[i], ")"),
                                      searchable = TRUE, page_size = 10))
            } else {
                cat("\n*Target GSEA file missing: ", tgt_tables[i], "*\n")
            }
            cat("\n\n")

            if (file.exists(ref_tables[i])) {
                ref <- read_metadata(ref_tables[i])[, c(-2, -3)]
                ref <- round_dataframe_columns(ref, digits = p$round_digits)
                print(captioned_table(ref, caption = paste0("Reference (", gsea_contrasts$reference[i], ")"),
                                      searchable = TRUE, page_size = 10))
            } else {
                cat("\n*Reference GSEA file missing: ", ref_tables[i], "*\n")
            }
            cat("\n")
        }
        cat(":::\n")
...
render_gprofiler2_section <- function() {
    ...
    for (i in seq_along(differential_names)) {
        ...
        if (!file.exists(table_path) || file.size(table_path) == 0) {
            cat(paste0("No ",
                       ifelse(p$gprofiler2_significant, "significantly", ""),
                       " enriched pathways were found for this contrast."))
        } else {
            all_enriched <- read.table(table_path, header = TRUE, sep = "\t", quote = "\"")
            all_enriched <- data.frame(
                "Pathway name"           = all_enriched$term_name,
                "Pathway code"           = all_enriched$term_id,
                "Differential features"  = all_enriched$intersection_size,
                "Pathway size"           = all_enriched$term_size,
                "Differential fraction"  = (all_enriched$intersection_size / all_enriched$term_size),
                "Adjusted p value"       = all_enriched$p_value,
                check.names = FALSE
            )
            all_enriched <- round_dataframe_columns(all_enriched, digits = p$round_digits)
            print(captioned_table(
                all_enriched,
...
contributors <- purrr::map_chr(simpleSplit(contributors, ";"), \(s) {
...
informative_variables <- informative_variables[!duplicated(purrr::map(
    rlang::set_names(informative_variables),
...
assay_files <- purrr::map(assay_names, \(x) params[[paste0(x, "_matrix")]])
assay_data  <- purrr::map(assay_files, \(x) {
...
pca_datas <- purrr::map(rlang::set_names(names(assay_data)), \(a) compilePCAData(assay_data[[a]]))
...
differential_results <- purrr::map(results, "diff_features")
warnings_list        <- purrr::list_c(purrr::map(results, "warnings"))
names(differential_results) <- differential_names
```

```{r differential-summary, echo=FALSE, warning=FALSE}
contrast_descriptions <- purrr::map_chr(seq_len(nrow(contrasts)), name_contrast)
...
sig_differential <- purrr::map(rlang::set_names(names(p_value_types)), \(pvt) {
    diff <- purrr::map(seq_len(nrow(contrasts)), \(x) {
...
differential_tables <- purrr::map(sig_differential, \(per_contrast) {
    do.call(rbind, purrr::map(per_contrast, \(updown) purrr::map(updown, nrow)))
})
```
...
contrasts_to_print$model <- purrr::map_chr(contrasts_to_print$Id, \(id) {
...
for (i in seq_len(nrow(contrasts))) render_de_details(i)
```
