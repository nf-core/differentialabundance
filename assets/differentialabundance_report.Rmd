...
for (i in 1:nrow(contrasts)){
  ...
  for (pvt in names(p_value_types)){
    ...
    # Prepare y data; include 0 p-values which will otherwise produce Inf because of log
    full_de$y_data <- sapply(full_de[[pval_column]], function(y) {ifelse(y==0,0,-log10(y))})
    inf_value <- ceiling(max(abs(full_de$y_data)) * 1.1)
    full_de$y_data[full_de$y_data==0] <- inf_value
    ...
    hline_thresholds[[paste(pval_column, '= 0')]] = inf_value
