...
Input was a matrix of `r nrow(assay_data$raw)` `r params$features_type`s for `r ncol(assay_data$raw)` `r params$observations_type`s`r ifelse(nrow(assay_data$normalised) < nrow(assay_data$raw), paste0(', reduced to ', nrow(assay_data$normalised), ' ', params$features_type, 's after filtering for low abundance. The matrix contains '), '')`.
...
```{r, echo=FALSE, results='asis', fig.height=8, eval=!is.null(params$features_biotype_col)}
...
rownames(assay_data_normalised) <- 1:nrow(assay_data_normalised)

features_df  <- read.csv(params$features, sep = "\t")

assay_data_normalised = assay_data_normalised %>% left_join(select(features_df, gene_id, gene_biotype), by=c("gene_id"="gene_id"))
biotype_counts = assay_data_normalised %>% group_by(gene_biotype) %>% count()


p = biotype_counts %>%
  ggplot(aes(x=gene_biotype, y=n)) + geom_bar(stat='identity') + theme_bw() + coord_flip()
print(p)
```
