...
for (i in 1:nrow(contrasts)){
    contrast_name <- paste0(contrasts$variable[i], ": ", contrasts$reference[i], " vs ", contrasts$target[i])

    if (!is.null(contrasts$blocking[i]) && contrasts$blocking[i] != "") {
        contrast_name <- paste0(contrast_name, " (blocking: ", contrasts$blocking[i], ")")
    }
    ...
    for (pvt in names(p_value_types)){
        ...
        if ("Gene biotype" %in% colnames(contrast_de)) {
            ...
            cat(paste0("No significantly differential '", dir, "' genes in ", contrast_name, ".\n\n"))
        }
        }
    }
}
