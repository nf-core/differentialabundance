...
```{r, echo=FALSE, results='asis', fig.height=8, eval=(!is.null(params$features_biotype_col) & !is.null(params$features))}
cat("### Number of differentially expressed genes by gene type")
cat("\nFrom the provided features annotation, the following plot was generated, showing the number of differentially expressed genes per gene type.")
assay_data_normalised <- as.data.frame(assay_data$normalised)
assay_data_normalised  <- cbind(gene_id = rownames(assay_data_normalised), assay_data_normalised)
features_df  <- read.csv(params$features, sep = "\t", header=T)

# Add additional columns called gene_id and gene_biotype to be able to join

if (! params$features_id_col %in% colnames(features_df)) {
  stop(paste0("Could not find ", params$features_id_col, " column in features file columns ", paste(colnames(features_df), collapse=", "), ", exiting..."))
}
if (! params$features_biotype_col %in% colnames(features_df)) {
  stop(paste0("Could not find ", params$features_biotype_col, " column in features file columns ", paste(colnames(features_df), collapse=", "), ", exiting..."))
} else {
  features_df$genetype <- features_df[[params$features_biotype_col]]
}
assay_data_normalised <- assay_data_normalised %>% left_join(select(features_df, params$features_id_col, genetype), by=c("gene_id"=params$features_id_col))
biotype_counts <- count(assay_data_normalised, genetype)
p <- biotype_counts %>%
  ggplot(aes(x=n, y=genetype)) + geom_bar(stat='identity') + theme_bw()
print(p)
```
