...
params:
  ...
  #  id: "study"
  #meta: NULL
  input_dir: "/Users/jonathanmanning/projects/nf-core-differentialabundance/work/62/2d2b7d76e5e2392e95db81ce4bf782"
  #input_dir: NULL
  ...
  normalised_matrix: "treatment-mCherry-hND6-sample_number.normalised_counts.tsv"
  #normalised_matrix: null
  #variance_stabilised_matrix: null                            # e.g. test_files/3_treatment-WT-P23H.vst.tsv
  variance_stabilised_matrix: "treatment-mCherry-hND6-sample_number.vst.tsv"
  contrasts_file: "SRP254919.contrasts.csv"
  #contrasts_file: null                                        # e.g. GSE156533.contrasts.csv
  samples_file: "SRP254919.samplesheet.sample_metadata.tsv"
  #samples_file: null                                          # GSE156533.samplesheet.csv
  #features_file: null
  features_file: "Mus_musculus.anno.feature_metadata.tsv"
  ...
  differential_file_suffix: ".deseq2.results.tsv" # This will be varialble once we get beyond RNA-seq
...
differential_results <- lapply(diff_prefixes, function(d){
  diff_file <- file.path(params$input_dir, paste0(d, params$differential_file_suffix))
...
differential_summary_string <- paste(
  paste0(nrow(contrasts), ' contrasts were examined for differential expression. '),
  paste0('Differential expression was assessed against a minimum fold change of ', params$min_fold_change, ' and a maximum adjusted p value of ', params$max_qval, '.'),
  paste(
  lapply(
    1:nrow(contrasts),
    function(x){
      signif <- differential_results[[x]][,params$differential_qval_column ] < params$max_qval  
      n_de_up <- length(
        which(
            differential_results[[x]][,params$differential_fc_column ] > log2(params$min_fold_change) &
            signif
        )
      )
      n_de_down <- length(
        which(
            differential_results[[x]][,params$differential_fc_column ] < log2(1/params$min_fold_change) &
            signif 
        )
      )
      
      paste0(
        "Contrast ", x, ' (', contrast_descriptions[x], ') ', "had ", n_de_up, ' ', paste0(params$feature_type, 's'), ' expressed significantly more highly in ', contrasts[x, 'target',], ' than ', contrasts[x, 'reference',], ' and ', n_de_down, ' expressed at sifnificantly lower levels.'
