...
if (length(grep(".html", enrichment_files, fixed=T))) {
  ...
  ifelse(params$gprofiler2_significant, paste0(" Enrichment was only considered if significant, i.e. adjusted p-value <=", params$gprofiler2_max_qval, "."), ""), "\n"))
  ...
  for (html in rev(grep(".html", enrichment_files, value=T, fixed=T))) {
    ...
    all_enriched <- read.table(file.path(params$input_dir, table), header=T, sep="\t")
