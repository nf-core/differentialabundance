...
An ANOVA test was used to determine assocations between continuous principal components and categorical covariates (including the variable of interest)

The results are illustrated below.

```{r, echo=FALSE, results='asis'}
pca_vs_meta <- anova_pca_metadata(pca_data$coords, samples, pca_data$percentVar)

d3heatmap::d3heatmap(
  -log10(pca_vs_meta),
  dendrogram = 'none',
  cellnote = pca_vs_meta,
  cexCol = 0.8, 
  cexRow = 0.8,
  height = (100 + (15 * nrow(pca_vs_meta))),
...
for (i in 1:nrow(contrasts)){
  ...
  full_de$differential_status[abs(full_de[[params$differential_fc_column]]) > log2(params$differential_min_fold_change) & full_de[params$differential_qval_column] < params$differential_max_qval] <- TRUE

  # Define the thresholds we'll draw

  hline_thresholds = vline_thresholds = list()
  hline_thresholds[[paste(params$differential_qval_column, '=', params$differential_max_qval)]] = -log10(params$differential_max_qval)
  ...
  plot_args <- list(
    ...
    y = -log10(full_de[[params$differential_qval_column]]),
    colorby = full_de$differential_status,
    ylab = paste("-log(10)", params$differential_qval_column),
