...
```{r, echo=FALSE}
observations <- read_metadata(file.path(params$input_dir, params$observations_file), id_col = params$observations_id_col)
...
pca_datas <- lapply(names(assay_data), function(assay_type){
  if (assay_type == params$exploratory_final_assay){
...
### Differential `r params$features_type` counts {.tabset}

```{r, echo=FALSE, results='asis'}
foo <- lapply(names(p_value_types), function(pvt){
  cat("\n#### ", pvt, "\n")
  print( htmltools::tagList(datatable(differential_tables[[pvt]], caption = paste0('Differential ',  params$features_type, ' counts (target relative to reference)'), options = list(dom = 't'), rownames = TRUE) ))
  cat("\n")
})
```
