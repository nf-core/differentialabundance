...
# Remove any informative variables that group samples the same way
informative_variables <- informative_variables[ ! duplicated(lapply(structure(informative_variables, names= informative_variables), function(x) as.numeric(factor(samples[[x]], levels=unique(samples[[x]])))))]
...
display_columns <- union(c(params$samples_id_col, unique(contrasts$variable)), informative_variables)
...
if (all(minimal_fetchngs_cols %in% colnames(samples))){
  ...
  additional_useful_cols <- colnames(samples)[which(apply(samples, 2, function(x) max(nchar(x))) <= 20)] 
}
...
pca_vs_meta <- anova_pca_metadata(pca_datas[['variance_stabilised']]$coords, samples[,informative_variables], pca_datas[['variance_stabilised']]$percentVar)
