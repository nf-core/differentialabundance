...
```{r, results='asis', echo=F, eval=!is.null(params$report_contacts_file)}
PIs <- paste(readLines(params$report_contacts_file), collapse="<br>")
PIs <- simpleSplit(PIs, "<br><br>")  # Split at double linebreaks to get list of PIs

if (length(PIs) > 1) {
  for (r in 1:length(PIs)) {
    if (r %% 2 != 0) {
      # This happens for the 1., 3. etc. entry which are put into the left column
      cat(paste0("<div class='div-container'>"))  # Open surrounding div that will contain 2 PIs next to each other
      cat(paste0("<div class='div-left'>**", PIs[r], "**</div>")) # Create inner left div
    } else {
      if (r <= length(PIs)) {
        cat(paste0("<div class='div-right'>**", PIs[r], "**</div>")) # Create inner right div; will skip this for the last row if number of PIs is uneven
      }
      cat(paste0("</div>"))
    }
  }
