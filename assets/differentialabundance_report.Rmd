...
params <- modifyList(params, test_params)
```
...
differential_summary_string <- paste(
  ...
  paste0('Differential expression was assessed against a minimum fold change of ', params$differential_min_fold_change, ' and a maximum adjusted p value of ', params$differential_max_qval, '.'),
  paste(
  lapply(
    1:nrow(contrasts),
    function(x){
...
opt <<- list(ntop = params$exploratory_n_features)
...
```{r, echo=FALSE, results='asis'}
if (params$study_type == 'rnaseq'){
  cat(paste0("Differential expression was assessed with DESeq2 version ", versions$`DESEQ2_DIFFERENTIAL.bioconductor-deseq2`, '.'))
}
```
